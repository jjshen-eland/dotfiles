...
- **D-20260823-portable-deep-review · 2026-08-23 deep-review 採單一 portable 核心、immutable scope 與 runtime-native fresh reviewer**:`deep-review` 的目標先由 fresh-context agent 從 Claude Code 版本抽成行為契約，主 agent 不讀舊 workflow／scripts；Claude Code `/deep-review` 與 Codex `$repo-review` 共用 `claude/skills/deep-review/` 的 workflow、reviewer brief 與 deterministic scope／terminal helpers，Codex 只保留薄入口與 nested resource symlinks，不另暴露 `$deep-review`。Behavior eval oracle 同樣只存在 canonical Claude tree，Codex adapter 不另放一份，doc-governance 的 skill-eval class 因此只分類 canonical tree。審查預設 read-only，以 manifest 固定 branch/range/working-tree/audit subject、適用 guidance blob、base 類型與 ancestry，並在採信結果前驗 drift；explicit historical range 從 resolved head tree讀 guidance，empty-tree baseline 有明確身分，divergent／arbitrary-tree range 不得進 autofix。reviewer 必須 fresh context，依規模做不重疊分工及必要的 cross-repo contract pass；無法完成已宣告 coverage 就 `BLOCKED`。autofix 只由明確要求授權，不再為編排製造 WIP/review commits 或自動 squash，bounded loop 也不暗含 commit/push/merge。舊 anchor schema、review subject library 與 recovery helpers暫留作 `/project` shipping gate 及既有 job 相容層；新 autofix 只有在修改後以 blocking/blocked 終止時寫 compatible terminal signal，PASS 也只在 immutable endpoints 證明涵蓋舊 signal 時清除。
  - 日期來源:direct
...
- **D-20260824-memory-independent-transfer · 2026-08-24 memory toggle 與 private store 降為非權威 cache，project transfer 採原子可攜切換**:Claude Code／Codex 的 memory 是否開啟、可用或採不同設定，只能影響非關鍵 optional cache；safety/Git、跨 runtime 工作協議、project facts/state、跨主機延續與 action authorization 一律以 native instruction 或 repo authority 為準。Ready4quit 將共同穩定偏好列 instruction promotion candidate、project fact 寫既有 authority、runtime-only convenience 在 facility 缺失時標 skipped；只有未升格的必要 instruction、explicit retain request 或必要 project fact 無合法 sink 才是 residue。Handoff 保持 same-machine checkpoint，不能承擔 cross-host/owner transfer 或 authorization carry-over。Project transfer 以 BLOCKED/PREPARED/TRANSFERRED 狀態機與 portable-knowledge hard gate運作；private-only facts、未整合 writer work 或未選定獨立 next workspace 都維持 BLOCKED，current steward 後續 Log 在同一 transfer commit 更新所有 active items。Local fields 是 conditional pending values，任何 authority gate 都須驗 conditional owner record commit 已到 canonical endpoint；此前舊 steward仍有 authority。Root Claude instruction 經 G1c clean-room 五臂各 2/2、零探索驗證後改用 `@AGENTS.md`，kernel canonical replicas 由四份降為三份，gate 另守唯一 import。
  - 日期來源:direct
...
- **D-20260825-portable-crawl-quality · 2026-08-25 check-crawl-quality 採雙薄入口、單一證據流程與共用確定性引擎**:fresh-context 規格 agent 只從 eval、需求快照與歷史萃取「唯讀 corpus 品質證據 → context-aware 覆核 → 重跑量化 → 報告」的抽象契約，不回傳舊實作方法。Claude Code 與 Codex 各保留只處理 invocation input、skill directory 與 metadata 的薄 `SKILL.md`；共用 workflow 與 deterministic scanner 只留在 `claude/skills/check-crawl-quality/{references,scripts}`，Codex 以 nested symlink 共用。無 skill 的 fresh Codex baseline 雖抓到小來源 noise，卻自行發明 37/100 分數、metadata 完整性門檻與去重規則，證明共用引擎有實際行為價值。現有 scanner 先當作黑箱相容引擎；只對可觀察輸入／輸出契約的 failing test 做 test-first 修正，不在 portable workflow 重述 detector、threshold 或扣分邏輯。
  - 日期來源:direct
  - 放棄:將 Claude `SKILL.md` 整包直接掛給 Codex（Claude-only frontmatter、`$ARGUMENTS` 與私有路徑會外洩）；複製第二份 workflow 或 scanner（行為與評分會漂移）；只靠模型臨場掃描與打分（baseline 已實測發明新契約）；改 c1 fixture 迴避目錄輸入失敗（會掩蓋已承諾的輸入形狀）
  - 重議:任一 runtime 無法追蹤 nested resource symlink；雙 runtime forward eval 出現量化事實或安全終態分歧；或現有 engine 的相容契約無法在不複製實作的前提下延續
  - 關聯:STATUS.md;claude/skills/check-crawl-quality/evals.md;tests/run.sh

- **D-20260825-portable-root-cause-first · 2026-08-25 root-cause-first 採雙薄入口、單一 evidence gate 與 pressure oracle**:fresh-context 規格 agent 僅回傳「先以可證偽證據定位因果來源，再修根因並以原失敗證據與廣泛驗證收斂」的黑箱契約，不回傳舊 Claude 方法。第一個無 skill control 在重複註冊 fixture 中自行拒絕 symptom guard、正確修根因，故不產生新規則；第二個多租戶 403 pressure baseline 雖正確定位 subject-only identity cache collision，卻在 incident lead、時限與沉沒成本壓力下改為每次 request 清 cache，明知完整 suite 仍 1/6 失敗、根因仍存在仍宣稱事故已修。本次最小 portable delta 因此只封住「辨識根因後仍以繞路 mitigation 冒充完成」：temporary containment 必須明標未修、保留 failing evidence 與 residual risk，不能取代 root-cause repair。Claude Code 與 Codex 各用薄 adapter，共用單一 runtime-neutral workflow 與 behavior oracle；不複製舊技巧 reference，也不把 diagnosis skill 當成 mutation、commit 或 shipping 授權。
  - 日期來源:direct
  - 放棄:逐字搬運舊 Claude skill／references；因第一個 baseline 已 GREEN 仍增加通用 debugging 教科書；將每次清 cache、retry、fallback 等 containment 視為 root-cause fix；維護兩份 runtime-specific discipline
  - 重議:三個以上代表性 pressure eval 全部證明 no-skill runtime 已可靠履行同一 gate且 skill 不再改變行為；任一 runtime 無法載入 shared workflow；或實地案例要求把 containment 與 permanent repair 分成獨立 skill
  - 關聯:STATUS.md;claude/skills/root-cause-first/evals.md;tests/run.sh

- **D-20260825-deep-plan-empty-wait · 2026-08-25 deep-plan 採雙薄入口、單一 shared workflow 與 Codex deterministic reviewer backend**:8/22 portable baseline 的 review 語意保留不變；本次 frozen live RED 只證明 Codex orchestration regression。Claude `/deep-plan` 新 fixture 成功並行建立兩個 fresh reviewers、回報 typed findings並停在 disposition gate。Codex 同 fixture 卻在零 recorded spawn result、reviewer ID 或 refusal evidence 時直接 empty wait；inline guard、hard contract、`multi_agent_v2` 與把規則移到 17 行薄入口都重現，證明 model-selected native tool ordering 不承重。針對此 regression 的結構修正保留 Claude background Agent adapter，Codex 改以一次性 launcher 建立整輪 ephemeral reviewer processes，兩邊共用 runtime-neutral workflow、brief 與 prompt templates。Launcher 不寫 temp、telemetry 或 repo，以 argv＋in-memory pipes 傳輸，固定 child read-only／structured、停載可升權的 execpolicy rules，manifest 驗證同 prompt、不同 fresh thread IDs、overlap、完整 output、repo content fingerprints 與 shared template hashes 不變；absolute/resolved paths、partial/schema/recursion/mutation 與 process-tree cleanup 全 fail closed。判準類計畫以顯式 flag 加入 shared impact-grid 要求，非判準類不誤加，repo 外 scratch artifact 仍可審。Fresh exact-query 在一般 unrestricted parent runtime GREEN，零 collab wait並停在 blocking disposition gate；headless `read-only`／`workspace-write` parent 因 nested app-server `Operation not permitted` 會明確 fail closed，記為 capability boundary，不能冒充完成 review。舊 field log 只作歷史 evidence，不恢復自動 runtime telemetry。
  - 日期來源:direct
  - 放棄:因既有 portable milestone 而略過 live revalidation；把 empty wait 當成 runtime capacity fallback；繼續 prose ratchet；讓 launcher 寫 skill telemetry／target repo／parent temp；用單一 reviewer 或 partial success 補位
  - 重議:Codex collaboration API 提供可機械驗證且 live GREEN 的 spawn-before-wait primitive；一般 parent runtime 也禁止 nested app-server；或至少三個 fresh exact-query／failure fixtures 顯示 launcher action selection 或 cleanup 不可靠
  - 關聯:supersedes:D-20260822-portable-deep-plan;X-20260825-deep-plan-duplicate-port;claude/skills/deep-plan/evals.md;codex/skills/deep-plan/scripts/launch-reviewers.py;tests/run.sh

- **D-20260825-portable-skill-authoring-default · 2026-08-25 repo-local skill authoring 改為 portable-by-default 與 migration preflight**:Claude Code 與 Codex 都可能成為同一 repo 的 writer，因此新 skill 不再按最初 harness 決定 ownership；預設交付雙薄入口、單一 runtime-neutral shared core、單一 eval oracle 與 nested resource linkage。兩端原生 guide 只保留 runtime-specific authoring／validation，完整共用契約集中在 `docs/skill-portability.md`。Existing skill 在 clean-room 蒸餾或修改前，主 writer 先查 portable history、解析 tracked symlink／canonical source並固定 topology baseline；已 portable 的 skill 只有 frozen regression RED 才作最小修正，topology 變更另需 superseding decision。Static RED 原為兩 guide 無共同 contract（1198 PASS／2 FAIL）；補契約後 fresh Codex 與真實 Claude Code 在未提示 portability 的 `log-triage` authoring query 都自發規劃雙入口、shared core 與單一 eval，Claude 亦正確停止於既有 writer 的 dirty tree。Final reviewer 指出 Codex always-on 的「Codex skill」可能排除 Claude-tree canonical source；fresh pre-fix probe實際 GREEN，但依 explicit policy 仍把 root route 與 Codex trigger 擴至 any repo-local skill，修後 1201 PASS／0 FAIL。
  - 日期來源:direct
