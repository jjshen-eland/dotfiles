...
- **M-20260825-portable-crawl-quality · 2026-08-25 Claude Code／Codex 共用 check-crawl-quality skill 完成**:`check-crawl-quality` 已拆成 shared evidence workflow、共用 deterministic scanner 與 Claude Code `/check-crawl-quality`、Codex `$check-crawl-quality` 兩個薄入口；Codex 以 nested symlink 共用 references/scripts，不複製 eval oracle。無 skill 的 fresh Codex baseline 自行發明 37/100 與額外 gates，確認 portable core 的行為價值；fresh forward eval 與真實 Claude Code CLI 最終都得到 records=120、sampled=120、special-report noise=80%、clean=90、rag=75、composite=84，所有 findings 均引用 engine example、所有來源均進摘要且 corpus digest 前後一致。獨立 clean-room reviewer 第二輪確認 runtime-neutral、無舊方法污染與雙 adapter 語意同義；Codex validator、Claude live discovery／slash invocation、doc-governance、shellcheck 與全 repo `tests/run.sh` 均通過，最終為 1179 PASS、0 FAIL。
  - 日期來源:direct
  - 放棄:直接複製 Claude skill 給 Codex；維護第二份 workflow／scanner／eval；允許 agent 自行算分或以 spot-check 補 finding evidence；只報有問題的來源而遺失 clean／unknown 狀態
  - 重議:nested resource symlink 在任一 runtime 失效；新格式或大 corpus 暴露 deterministic evidence／sampling／per-source 漂移；或雙 runtime forward eval 出現量化與安全終態分歧
  - 關聯:D-20260825-portable-crawl-quality;claude/skills/check-crawl-quality/evals.md;tests/run.sh

- **M-20260825-portable-root-cause-first · 2026-08-25 Claude Code／Codex 共用 root-cause-first skill 完成**:`root-cause-first` 已收斂為單一 runtime-neutral evidence workflow、Claude Code `/root-cause-first` 與 Codex `$root-cause-first` 兩個薄入口；Codex 以 nested symlink 共用 canonical references，不複製 eval oracle，舊 method-specific reference 內容退場並只保留未被 adapter 載入的 compatibility pointer。真正的無 skill RED 是 fresh Codex 雖正確定位多租戶 identity cache collision，仍因 scope 壓力改成每個 request 清 cache，且在完整 suite 仍 1/6 失敗時宣稱事故路徑已修；有 skill forward eval 改為誠實 `BLOCKED`、拒絕 symptom workaround 並保持 source 唯讀。另兩個 pressure controls 與不可重播但有充分 trace/control 的 positive control 均 PASS；三輪獨立 clean-room review 最終確認 reproduction wording、runtime neutrality、adapter parity 與實體 linkage 無 finding。雙 runtime validator、shellcheck、doc-governance 與全 repo `tests/run.sh` 均通過，最終為 1185 PASS、0 FAIL。
  - 日期來源:direct
  - 放棄:直接複製舊 Claude debugging 方法；維護兩份 runtime-specific workflow；把 cache clear、權限放寬或 boundary guard 冒充 root-cause repair；把不可重播本身誤作 evidence 不足
  - 重議:nested reference symlink 在任一 runtime 失效；新的 pressure eval 再出現 containment-as-repair／false completion；或一次性 incident 的 trace/control evidence 無法得到一致終態
  - 關聯:D-20260825-portable-root-cause-first;claude/skills/root-cause-first/evals.md;tests/run.sh

- **M-20260825-portable-deep-plan-revalidation · 2026-08-25 deep-plan V2 精華與 portable-by-default authoring 完成**:8/22 portable baseline 的 review 語意完整保留；重複 migration 的分類錯誤另記 X-20260825。真正的 regression 是 Codex native collaboration 在四個 fresh exact-query traces 都以零 reviewer ID empty-wait，因此只針對此 RED 改成 Claude/Codex 雙薄入口、單一 shared workflow／brief／prompt templates 與 Codex in-memory deterministic launcher。Launcher 一次建立兩個 structured、ephemeral、read-only reviewers，並以 absolute/resolved path gates、execpolicy isolation、repo content fingerprints、shared artifact hashes、typed complete-set validation、recursion guard 與 POSIX/Windows process-tree cleanup fail closed；Claude 對 partial/malformed result 同樣停止。Final Codex exact-query 得到兩個 fresh IDs、同 prompt SHA、overlap 與全狀態 unchanged，Claude live probe 也停在 disposition gate；兩位 fresh final reviewers 的 efficiency findings 為零，portable-authoring P1 經 always-on trigger 修正後 CLOSED。另建立單一 `docs/skill-portability.md`，兩端 guide 強制載入；未提示 portability 的 fresh Codex 與真實 Claude Code authoring probes 都自發產生雙入口＋shared core。雙 validator、doc audit、kernel/diff checks 與全 repo `tests/run.sh` 均通過，最終為 1201 PASS、0 FAIL。
  - 日期來源:direct
