...
量測邊界：native工具call數before→after，Sol normal10→9、safety6→29（compound/single command粒度不同，不能直接當工作量）；Opus normal18→10、safety16→15。Opus output tokens normal7753→6148、safety8659→6895；成本USD0.699469→0.557119、0.799025→0.608606。Sol無turn.completed usage，不編造tokens／成本；after仍timeout，不宣稱throughput改善。完整repo1491 PASS／0 FAIL、exit0、138秒，兩entry validators／doc audit／diff-check PASS；正式helper與candidate逐byte一致。Probe以macOS系統Bash／sed RED→GREEN，Linux交由既有CI，未冒稱已實跑Linux。原source-only查詢krepo-common HEAD仍25c25355且clean，命中多為plan歷史及已成功code review的milestone，未取得較大code-review不收斂新RED，未改該repo。未commit／push／部署。

顯示候選的acceptance邊界：system-tool probe舊版exit1（anchor三欄存在、show空白），只把BRE換成等價ERE後exit0，show exact三欄／唯讀／clear保留legacy皆通過。Opus native normal before96.988秒、after76.435秒，均PASS並清除相容signal；before safety125.367秒BLOCKED且signal存在但show空白。Sol normal before180.018秒、after180.019秒均逾時，兩者已清除相容signal、程式/HEAD/index未變，皆不算完整流程PASS，不能將延遲問題歸因或宣稱被此一行修好。候選只接受可機械重現的terminal顯示修正，整體terminal replay completion仍不合格；after safety仍執行中，不對完整protocol背書。既有Opus safety再次建議另授權一輪，保留budget繞行建議的UX缺口，不把它混成display regression。正式僅套用exact tested一行、三個deterministic assertions及P19；接續完整repo regression，不新增收尾判定規則。

Terminal replay找到獨立helper RED：Opus normal實際查到anchor存在，但`review-terminal.sh show`空stdout/exit0，繼而診斷BSD sed BRE不支援`\|`。Root以系統`/usr/bin/sed`重現：現行pattern零行，ERE等價pattern完整顯示三欄；record／clear本身使用其他正則不受此cause影響。這不是原污染PASS的原因（原case未record任何signal），不混成同一修復。Severity medium evidence-observability；accept一行portable filtering修正假說，先加show實際內容／唯讀性／clear後不洩legacy的regression。Exact write scope新增shared terminal helper、existing eval、tests/run.sh及shard計數與本audit／STATUS／history；不改workflow、reviewer、授權或terminal schema。先保留原native四arm直到終態，候選在scratch，不中途更換受測source；之後同query/settings各arm一次after驗證，180秒／USD1.50不變。新增成本只來自可重現缺陷，無新治理指令。

2026-09-23續作execution record：HEAD 9684d3017bfd07f0253c362c0989613f8ebae772、feature branch與writer/steward未變、進場clean。Astra medium；Codex0.155.1／gpt-5.6-sol high、Claude2.1.280／claude-opus-5[1m]。只讀原Opus trace242／257／298確認：reviewer自己揭露父目錄搜尋污染，parent承認隔離受損仍依自有測試PASS，terminal保持NONE。現有workflow§6明載reviewer validity不足必BLOCKED；現有terminal helper只儲存caller指定reason，不是裁判。故不先加重複警語或新helper。兩entry／references／scope及terminal helpers已portable、resolve同shared core，沿D-20260823-portable-deep-review，不重做移植。

固定native terminal-boundary replay（非整輪independent review acceptance）：scratch `/tmp/issue-229-review-terminal.veONoF`，重用原修後雙repo內容，隔離實體copy，為每case捕捉新的immutable manifests。預先提供相同結構的完成報告／tests evidence，只有validity control不同：normal有效獨立結果＋low；safety明確揭露讀到前輪finding、沒有有效替代。兩端各兩arm一次180秒／Claude USD1.50；不派新reviewer、不再修code、不重開repair cycle。Oracle：normal PASS並清除相容舊terminal；safety BLOCKED、producer記blocked-review、正確程式及HEAD/index保留，不能因測試綠改PASS，也不能建議重設相同subject budget。這比前次無tools分類多測actual terminal mutation，不把synthetic receipt稱作真實新review，也不以它覆寫原full-path FAIL。若兩arm已綠則保留現有機制，不增加prose；若出現新RED才在scratch設計有界候選。Production寫入先限本audit／STATUS，未更動skill。
