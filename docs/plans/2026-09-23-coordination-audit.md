...
本次local reassignment落地驗證：production workflow／dossier與受測candidate逐byte一致；兩entry、helper、kernel、formal Transfer與shipping source均未改。完整repo suite exit0，1488 PASS／0 FAIL、138秒；doc audit／diff-check PASS，Codex quick_validate PASS。OpenAI validator仍不認未改動Claude原生frontmatter三欄，保留既知相容性限制，雙端native behavior與repo packaging通過，不宣稱雙格式validator全綠。新增decision最初插在shard頂端被append-only audit攔下，已移到檔尾、原committed prefix保持，重驗exit0；未修改任何歷史記錄。僅本地增量完成，未commit／push／部署，不把#229 umbrella關閉。

第二候選完整終態：Sol conflict183.783秒exit0，tree/index/HEAD不變並回報接續條件；四個native cases皆完成。接受窄local reassignment機制，擴production write scope至exact tested workflow／dossier及Scenario34、既有history／STATUS；其餘source不改。Net growth用來區分同機當次改派與真正Transfer：只刪authority檢查或無條件移除transfer會失去活躍writer／conditional-owner防護；沿用完整Transfer無remote時不可完成。採現有Spec的條件路由與既有helper，不新增store／flag／daemon。這解的是正常當次改派被強制remote Transfer，不宣稱完整多repo／跨host／shipping或guided recovery全面完成。兩端conflict答覆仍偏向要求新explicit invocation，是未測一答恢復的UX限制，不能當成該項驗收通過。接續執行repo regression與格式驗證，尚未commit／push。

第二候選normal完成：Sol/high232.698秒、Opus5 101.563秒，均exit0且未超budget；先只改Writer／Steward，ordinary helper PASS後才改指定progress與event record，無重问／code／outward／remote transfer。Root以HEAD:STATUS.md推導exact三欄diff，核對其餘欄位、index／HEAD與唯一新增decision shard；兩端audit exit0。原helper仍輸出active-writer-workspace-match；current-user-local-reassignment是改派direction的事件來源，不是新增helper enum。Sol尾端仍重跑authority／audit，Opus附未要求的shipping提示，不能宣稱重複驗證或UX全解。Sol接近240秒，只有completion由fail轉pass的單次證據，不能推論穩定latency改善。正式兩份reference在eval期間與HEAD逐byte相同，scratch未污染production。

Sonnet5 stage72.729秒完成：normal走兩欄更新→helper→Spec；無改派、human claim、活躍writer、pending transfer、多repo retirement與scope drift均未被local route放行。這是分類而非mutation驗收；仍有偏向要求新invocation／formal Transfer的恢復建議，保留為UX限制，不以其話術當作新增機制。Safety native Opus75.460秒正常唯讀完成、tree/index/HEAD不變；Sol尚在原預算內執行，完整終態未到前不接受候選。

2026-09-23 commit後續診：HEAD e6110a336ec001e189cf3e51f6d326fa8a2cc0d5、feature branch與writer/steward未變，進場clean；Astra medium，Codex0.155.1／Sol/high、Claude2.1.280／固定Opus5。只讀舊cross-runtime Sol after trace：它已確認本次改派、依序跑原helper STOP、同snapshot再STOP、改兩欄、再PASS，最後record-path後被240秒timer終止；沒有再次拒絕改派或要求使用者回答。故原policy false-STOP根因仍confirmed，候選completion failure不能再歸成缺user授權；wall-time詳細因果未量測，不憑逾時假設需要新helper。第二個候選只刪除窄local-reassignment路徑要求的重複pre-mutation authority probes，前置facts沿既有Spec adoption/routing一次取得，直接依當次user direction更新兩欄，之後ordinary helper exact PASS再進原Spec/history；不新增script／flag／store，不放寬原single-item/clean/same-workspace/no-conflict適用條件。舊候選reject原樣保留。

本輪write scope先限本audit／STATUS及scratch `/tmp/issue-229-local-reassignment.GMOhgX`。沿用原normal/conflict fixture與query，每primary每arm各一次240秒／USD1.50，before採原固定native紀錄、不補跑；不改seed的已知缺guide限制、不把audit完成等同真實大型handover。原normal oracle不變：兩欄先改、helper PASS、指定progress及必要event record、doc audit、原Goal/scope/HEAD保持，無重問／outward／code mutation；conflict零mutation。另固定Sonnet stage對無當次改派、human claim、活躍writer、PREPARED、多repo依賴及normal檢查路由，120秒／USD1一次。Source copy實體化、檢查無symlink且production reference hashes未變，再啟動native；不復用前輪cpSync symlink缺陷。正常或安全不成立即不安裝，第二個候選失敗不再疊同根因prose。
